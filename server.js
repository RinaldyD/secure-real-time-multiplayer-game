...
let playersList = [];
let [shrimpX, shrimpY] = getRandomPosition();
let shrimp = new Collectible({ x: shrimpX, y: shrimpY, value: 1, id: Date.now() })
let [sharkX, sharkY] = getRandomPosition();
let shark = new Player({ x: sharkX, y: sharkY, value: 1, id: Date.now() });
let connections = [];
...
io.sockets.on('connection', socket => {
  ...
  socket.emit('init', { id: socket.id, players: playersList, shrimp: shrimp, shark: shark });
  ...
  socket.on('update', (updatedUser) => {
    ...
    io.emit('update', { players: playersList, shark: shark, shrimp: shrimp, player: null });
  });
...
function tick() {
  // move the spike
  if (shark.x + shark.radius > dimension.maxX) vx = -vx;
  if (shark.y + shark.radius > dimension.maxY) vy = -vy;
  if (shark.x - shark.radius <= dimension.minX) vx = -vx;
  if (shark.y - shark.radius <= dimension.minY) vy = -vy;
  shark.x += vx;
  shark.y += vy;
  let playerUpdate = null

  playersList.forEach(player => {
    if (shark.collision(player)) {
      let [positionX, positionY] = getRandomPosition();
      player.x = positionX;
      player.y = positionY;
      player.score = 0;
    ...
    if (p.collision(shrimp)) {
      player.score += 1;
      player.radius += 5;
      let [shrimpX, shrimpY] = getRandomPosition();
      shrimp = new Collectible({ x: shrimpX, y: shrimpY, value: 1, id: Date.now() })
      playerUpdate = player;
